...
while True: 
  #textMsg=input("enter the filename :")
  #romeo.txt
  #todo filehander to accept any *.txt
  ...
  for line in filehandler:
    ...
    for word in line:
      word = word.split()
      newMsg += newMsg.join(word)
      for letter in newMsg:
        if letter in upperBinary:
          comMsg += upperBinary[letter]
        elif letter in lowerBinary:
          comMsg += lowerBinary[letter]
  print(comMsg)
  retry = input("\ntry again? Y/N : ").upper()
  if retry == "Y":
    continue
